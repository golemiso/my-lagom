...
/**
  * This is an event sourced entity. It has a state, [[MylagomState]], which
  ...
  */
class MylagomEntity extends PersistentEntity {

  override type Command = MylagomCommand[_]
  override type Event = MylagomEvent
  override type State = MylagomState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: MylagomState = MylagomState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case MylagomState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        MylagomState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class MylagomState(message: String, timestamp: String)
...
object MylagomState {
  /**
    ...
    */
  implicit val format: Format[MylagomState] = Json.format
}

/**
  * This interface defines all the events that the MylagomEntity supports.
  */
sealed trait MylagomEvent extends AggregateEvent[MylagomEvent] {
  def aggregateTag = MylagomEvent.Tag
}

object MylagomEvent {
  val Tag = AggregateEventTag[MylagomEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MylagomEvent
...
/**
  * This interface defines all the commands that the MylagomEntity supports.
  */
sealed trait MylagomCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends MylagomCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends MylagomCommand[String]
...
/**
  ...
  * application loader.
  */
object MylagomSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MylagomState]
  )
}
